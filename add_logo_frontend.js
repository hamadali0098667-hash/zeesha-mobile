    ...
    <div className="max-w-5xl mx-auto">
      ...
      {user.role === 'admin' && (
        <div className="bg-white dark:bg-gray-800 shadow-sm sm:rounded-xl p-6 mb-8 border border-gray-100 dark:border-gray-700">
          <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Shop Branding</h2>
          <form onSubmit={handleSaveSettings} className="space-y-6">
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Shop Name</label>
              <input type="text" className="w-full md:w-1/2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white" value={settings.shopName} onChange={e=>setSettings({...settings, shopName: e.target.value})} />
            </div>
            <div>
              ...
              <div className="flex items-center gap-6">
                {(logoFile || settings.shopLogo) ? (
                  <img src={logoFile ? URL.createObjectURL(logoFile) : \`http://localhost:5000\${settings.shopLogo}\`} alt="Shop Logo" className="h-20 w-20 object-contain bg-gray-100 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-1" />
                ) : (
                  <div className="h-20 w-20 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center text-gray-400 border border-dashed border-gray-300 dark:border-gray-600">No Logo</div>
                )}
                <input type="file" accept="image/*" onChange={e => setLogoFile(e.target.files[0])} className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100" />
              </div>
            </div>
            ...
            {staff.map(s => (
              <li key={s._id} className="p-4 flex flex-col sm:flex-row justify-between sm:items-center hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors gap-4">
                ...
                <div className="flex items-center gap-4">
                  <span className={\`px-3 py-1 text-xs rounded-full font-bold shadow-sm \${s.isActive ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'}\`}>
                    {s.isActive ? 'Active' : 'Inactive'}
                  </span>
                  {user._id !== s._id && user.role === 'admin' && (
                    <button onClick={() => toggleStatus(s._id, s.isActive)} className="text-sm font-medium underline text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
...
fs.writeFileSync(path.join(srcPath, 'pages', 'Settings.jsx'), settingsJsx.trim());

// 2. Fetch and apply settings in Sidebar.jsx
let sidebarJsx = fs.readFileSync(path.join(srcPath, 'components', 'Sidebar.jsx'), 'utf8');
if(!sidebarJsx.includes('shopSettings')) {
  ...
  sidebarJsx = sidebarJsx.replace(
    ...
    `{shopSettings.shopLogo ? (
          <img src={\`http://localhost:5000\${shopSettings.shopLogo}\`} alt="Logo" className="h-10 object-contain mr-2" />
        ) : null}
...
// 3. We will handle Inventory.jsx image upload in another block
console.log('Settings and Sidebar updated for shop logo.');
