    ...
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 shadow sm:rounded-md p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => { setShowAddPurchase(!showAddPurchase); setEditingPurchaseId(null); setNewPurchase({ supplier: '', product: '', quantity: '', cost: '' }); }} className="bg-indigo-600 text-white px-3 py-1 rounded">New Purchase</button>
        </div>
        ...
        {showAddPurchase && (
          <form onSubmit={handleAddPurchase} className="mb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-gray-50 dark:bg-gray-900 p-4 rounded">
            ...
            <button type="submit" className="bg-green-600 text-white px-4 py-2 rounded col-span-1 md:col-span-2 lg:col-span-4">{editingPurchaseId ? 'Update Purchase & Adjust Stock' : 'Complete Restock & Add to Inventory'}</button>
          </form>
        )}
